   ...
   page_require_level(3);
	...
	if($action == 'ajax'){
		// escaping, additionally removing everything that could be (html/javascript-) code
		 ...
		 $sucursal_inicio=$_POST['sucursal_inicio'];
		 $aColumns = array('name');//Columnas de busqueda
		 $sTable = "products ";
		 $sWhere = "WHERE id_sucursal=$sucursal_inicio ";
		if ( $_POST['q'] != "" )
		{
			$sWhere .= "and (";
				...
				while ($row=$db->fetch_array($query)){
					...
					<tr>
						...
						<input type="text"  id="precio_venta_<?php echo $id_producto; ?>"  value="<?php echo $precio_venta;?>"  hidden>
							<label><?php if($id_categoria=='18'){echo "UNIDAD S/. ".$precio_unitario;}else{ echo " UNIDAD  S/. ".$precio_unitario."<br>
								BLISTER  S/. ".$precio_por_blister."<br>
								CAJA  S/. ".$precio_venta."<br>" ;}?></label>
						</div></td>
						<td class='text-center'><a class='btn btn-info'href="#" onclick="agregar1('<?php echo $id_producto ?>')"><i class="glyphicon glyphicon-plus"></i></a></td>
					</tr>
					<?php
